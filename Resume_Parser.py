...
SECTORS = {
    "IT": ["python","java","C","C++", "html", "css", "spftware development", "data analytics", "cloud computing", "AI", "machine learning"],
    "Design": ["UI/UX","figma", "Adobe", "sketch", "design"],
    "Finance": ["accounting", "financial analysis", "financial modeling", "financial planning","investment", "banking", "finance"],
    "Sales": ["sales","marketing","client relationship","business development"]
...
def extract_infomation(text):
    text_lines = text.splitlines()
    text_lower = text.lower()
    
    #to match phone numbers and emails
    
    phone_numbers = re.findall(r"\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}", text)
    emails = re.findall(r"[a-zA-Z0-9._x+-]+@[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}",text)
    
    name == "Not Found"
    for line in text_lines[:5]:
        if len(line.strip()) > 2 and not any(keyword  in line.lower() for keyword in ["email", "phone", "linkdin", "github", "address"]):
    ...
    return {
        ...
        "Projects": projects,
        "Matched_sectors": matched_sectors if matched_sectors else ["No sector match found"]
        
    }
    
def parse_resume(file_path):
        text = extract_text_from_pdf(file_path)
        return extract_infomation(text) if text else{}
    
    #allows to browse .pdf files
...
def display_resume(parsed_resume):
        ...
        output_text.instert(tk.END, "Projects:\n")
